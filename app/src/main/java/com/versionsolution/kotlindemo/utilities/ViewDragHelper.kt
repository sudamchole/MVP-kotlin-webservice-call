 ...
 */
private constructor(context: Context, private val mParentView: ViewGroup?, interpolator: Interpolator?, private val mCallback: Callback?) {
   ...
   open abstract class Callback {
    ...
    private fun getEdgesTouched(x: Int, y: Int): Int {
        ...
        if (x < mParentView!!.getLeft() + edgeSize) result = result or EDGE_LEFT
        if (y < mParentView!!.getTop() + edgeSize) result = result or EDGE_TOP
        if (x > mParentView!!.getRight() - edgeSize) result = result or EDGE_RIGHT
        if (y > mParentView!!.getBottom() - edgeSize) result = result or EDGE_BOTTOM
